...
description: "Use this skill to launch, stop, and poll target applications with Drive.NET. Covers the 'lifecycle' MCP tool for `launch`, `stop`, and `status`, including startupWaitMs, singleInstance reuse or restart, pre-launch sessionStartWarning control, graceful shutdown, and pairing launch flows with `session connectWait` or `session reconnect`. Keywords: Drive.NET, lifecycle, launch, stop, status, startupWaitMs, singleInstance, reuse, restart, sessionStartWarning, gracePeriodMs, connectWait, reconnect, process control."
...
- `sessionStartWarning` controls the same safety notice used by Drive.NET session attach flows.
